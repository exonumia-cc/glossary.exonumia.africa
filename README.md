...
[glossary.exonumia.africa](https://glossary.exonumia.africa). A landing page (`index.html`)
explains the glossary and opens the dictionary itself (`peruse.html`), where every term occupies
one row with a dedicated column per language, so translations can be read and compared side by
side.
...
Then open <http://localhost:4173> for the landing page, or <http://localhost:4173/peruse.html>
for the dictionary. Deployment is a plain upload of the repository root to any static host.

## Files

| File | Purpose |
| --- | --- |
| `index.html` | Landing page — what the glossary is, how to use it, how to contribute |
| `styles.css`, `script.js` | Landing page styles and its mobile menu toggle |
| `peruse.html` | Dictionary page shell — masthead, toolbar, sidebar, footer |
| `peruse.css` | Dictionary theme, light and dark, responsive |
| `app.js` | Loads the JSON, builds the model, renders and filters the dictionary |
...
Everything is static, so any host will do. One caveat: `app.js` and `peruse.css` are referenced
